...
p('У варианта указаны цена в минутах дня и изменение доверия бизнеса.');
p();
...
p('пайплайн **сборка → тесты → выкладка → прод**. Модуль без изъяна проходит все стадии,');
p('модуль с изъяном падает — на какой стадии, решает случай, а на экране игрока');
p('появляется разбор: что случилось, из-за какого решения и как надо было.');
p();
...
for (const round of ROUNDS) {
  ...
  p('Задач в пуле ' + round.pool.length + ', команда играет ' + round.perTeam + '. ' +
    'Таймер ' + round.minutes + ' мин, бюджет ' + round.budget + ' минут дня ' +
